...
class [[eosio::contract("userstorage")]] userstorage : public eosio::contract {

public:
  
  userstorage(name receiver, name code,  datastream<const char*> ds): contract(receiver, code, ds) {}

  [[eosio::action]]
  void append(name id, std::string role, time_t timestamp, std::vector<name> net_user, std::vector<name> net_subuser) {
    require_auth( id );
