...
public:
  ...
  [[eosio::action]]
  void newlist(name user, int numlist,
    ...
    {
        lists_index table( get_self(), get_first_receiver().value );
        auto iterator = table.find(numlist);
        if( iterator == table.end() ) {
          table.emplace(user, [&]( auto& row ) {
            row.numlist = numlist;
            row.nAccMax = nAccMax;
            row.durMaxAcc = durMaxAcc;
            row.durPru = durPru;
            row.sRes = sRes;
            row.nTAT = nTAT;
            row.nAT = nAT;
            row.vNAccR = vNAccR;
            row.vNR = vNR;
            row.action_p_action_p_restriction = action_p_action_p_restriction;
            row.x_axis = x_axis;
            row.y_axis = y_axis;
          });
        } else {
  ...
  [[eosio::action]]
  void open(name user, int numlist) {
    require_auth( user );
...
///////////////////////////////
    ...
    lists_index lists( get_self(), get_first_receiver().value);

    auto iterator = lists.find(numlist);
    check(iterator != lists.end(), "This list does not exist");
    auto currentlist = lists.get(numlist);
    
    int nAccMax = currentlist.nAccMax;
    int durMaxAcc = currentlist.durMaxAcc;
    int durPru = currentlist.durPru;
    int sRes = currentlist.sRes;
    vector<int> nTAT = currentlist.nTAT;
    int nAT = currentlist.nAT;
    vector<int> vNAccR = currentlist.vNAccR;
    vector<int> vNR = currentlist.vNR;
    vector<int> action_p_action_p_restriction = currentlist.action_p_action_p_restriction;
    int x_axis = currentlist.x_axis;
    int y_axis = currentlist.y_axis;
...
///////////////////////////////////////////////////////////////////
  ...
  [[eosio::action]]
  void upload(int request, name user, std::string evidence) {
    require_auth(user);
...
private:
  ...
  struct [[eosio::table]] lists {
    ...
    int x_axis;
    int y_axis;
    uint64_t primary_key() const { return numlist; }
  };
  typedef eosio::multi_index<"lists"_n, lists> lists_index;
